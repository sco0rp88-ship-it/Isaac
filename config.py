...
def _provider_defaults_from_env() -> dict[str, ProviderConfig]:
    return {
        ...
        "xai": ProviderConfig(
            ...
            provider_type="openai_compat",
            api_key=(os.getenv("XAI_API_KEY") or os.getenv("GROK_API_KEY") or ""),
            ...
            rpm=60,
            tpm=100_000,
            timeout=_int(os.getenv("XAI_TIMEOUT", "90"), 90),
        ),
        "grok": ProviderConfig(
            provider_id="grok",
            display_name="Grok (xAI alias)",
            provider_type="openai_compat",
            api_key=(os.getenv("GROK_API_KEY") or os.getenv("XAI_API_KEY") or ""),
            base_url=os.getenv("XAI_BASE_URL", "https://api.x.ai/v1/chat/completions"),
            model=os.getenv("GROK_MODEL", os.getenv("XAI_MODEL", "grok-3-mini")),
            rpm=60,
            tpm=100_000,
            timeout=_int(os.getenv("XAI_TIMEOUT", "90"), 90),
        ),
